...
class DeepSeekClient:
    ...
    def chat_completion(
        ...
        try:
            ...
            logger.info(f"API密钥前10位: {self.api_key[:10]}...")
            ...
            if "timeout" in error_message.lower():
                ...
                raise Exception(f"API请求发生错误: {error_message}")
...
def chat():
    ...
    try:
        response = deepseek_client.chat_completion(
            message=message, max_tokens=int(max_tokens), temperature=float(temperature)
        )
        ...
        elif "网络连接" in error_message:
            return jsonify({"error": "网络连接失败，请检查网络连接"}), 503
        elif "API密钥无效" in error_message:
            return jsonify({"error": "API密钥配置错误，请联系管理员"}), 401
        elif "频率过高" in error_message:
